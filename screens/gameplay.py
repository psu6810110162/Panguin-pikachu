...
# ============================
# Pause Overlay — ส่งต่อ touch เมื่อซ่อนอยู่
# ============================
class PauseOverlay(FloatLayout):
    def on_touch_down(self, touch):
        if self.opacity == 0:
            return False   # ซ่อนอยู่ → ไม่กิน touch ✅
...
class GamePlayScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        # ── Pause Overlay ──
        self.pause_overlay = PauseOverlay(opacity=0, disabled=True)  # ✅ ใช้ PauseOverlay
    ...
    def _move(self, direction):
        ...
        if self.grid.is_on_path(new_col, new_row):
            ...
            if self.path_index == len(self.grid.path) - 1:
                logger.info(f"ชนะแล้ว! ระยะ {self.grid.get_distance_m()} m")
    ...
    def _on_keyboard_down(self, keyboard, keycode, text, modifiers):
        if self.penguin.is_dead:
            return False
        if not self.game_event:       # pause อยู่ → ไม่รับ input
            return False
        if keycode[1] == 'left':
