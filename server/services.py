...
def create_session() -> SessionModel:
    ...
    session = SessionModel(room_code=generate_room_code(), teacher_token=secrets.token_urlsafe(32))
...
def ingest_signed_run(
    ...
    try:
        ...
        db.session.rollback()
        run = db.session.query(RunModel).filter_by(run_id=record.run_id).one()
        _apply_result(run, record, result, player_name)
        db.session.commit()
    return run
