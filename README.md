...
เปิดคู่มือ **HOW TO PLAY** จากหน้า Main Menu หรือกดปุ่ม **?** ข้างปุ่ม Pause ระหว่างเล่นเกม คู่มือในเกมจะแสดงกติกาตามข้อมูล balance เวอร์ชันปัจจุบัน ได้แก่

- เป้าหมาย 1,000m, การควบคุม ←/→ และ Spacebar สำหรับ Eco-Seed
- Hearts, checkpoint respawn, Heat Meter และ Capitalist Anger
- หลักฐานวิทยาศาสตร์, ทางแยกนโยบายครบ 10 จุด, และ Carbon Baron 3 เวฟ
- คะแนน Gameplay/Survival, Run Reduction, Cognitive, Net Impact และ DAG Report Card

แหล่งอ้างอิงเชิงออกแบบคือ [docs/GAME_DESIGN.md](docs/GAME_DESIGN.md) (GDD V.2); ค่าตัวเลขและเนื้อหาทางแยก/บอสจริงอยู่ใน `balance/v1/*.json`
...
| `STEALTH_ASSESSMENT_ENABLED` | `0` (ปิด) | persist/แสดง `net_impact_score`/`cognitive_score`/`rank` บน leaderboard/dashboard — evaluator คำนวณค่าพวกนี้เสมอไม่ว่าธงจะเป็นอะไร (ดู docs/adr/012) |
...
server/             # Flask backend — import ได้เฉพาะ core/ (ดู server/requirements.txt แยกจากเกม)
  ...
  config.py            อ่าน env var (DATABASE_URL/SYNC_SECRET/PORT/RATE_LIMIT/FLASK_DEBUG/STEALTH_ASSESSMENT_ENABLED) จุดเดียว + guard default secret
