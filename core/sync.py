...
class HttpTransport:
    ...
    def send(self, payload: SignedPayload) -> bool:
        ...
        try:
            ...
            #
            # ยกเว้น 429: เป็น 4xx ตัวเดียวที่ transient — rate limiter ของ /api/v1 ตอบ
            # ค่านี้จริงเมื่อ client โดน throttle ถ้า treat เป็น "จัดการแล้ว" run จะถูก
            # นับว่า sync สำเร็จทั้งที่ server ไม่เคยรับ = คะแนนนักเรียนหายถาวรเงียบ ๆ
