...
> **สถานะจริงในโค้ด ณ ตอนนี้ (หลัง PR1+PR2 merge):** มีแค่ docs + `balance/v1/*.json` — ยังไม่มีโค้ดไหนอ่านไฟล์เหล่านี้เลย `core/junction_data.py`, `core/meters.py` (bar UI), `core/scoring/stealth.py`, `core/scoring/dag.py`, `screens/report.py` **ยังไม่ถูกสร้าง**

**Story D1-B1 · Y-Junction Content Dataset** `p0` `data` — ✅ **data เสร็จ (PR2)** / ⬜ **loader ยังไม่เขียน**
สร้าง `balance/v1/junctions.json` — 10 ทางแยกครบ: สถานการณ์, ตัวเลือกซ้าย/ขวา, stat delta, category, systemic-flag
- Task: ~~schema + data~~ ✅ · **`core/junction_data.py` loader** ⬜ (ยังต้องเขียน) · ~~content validation test~~ ✅ (`tests/test_balance.py`)
- ✅ Acceptance: `get_junction(zone_id)` โหลดครบ 10 index — **ยังไม่ผ่าน เพราะ loader ยังไม่มี**

**Story D1-B2 · Evidence-to-Stats Item System** `p0` `game` — ⬜ ยังไม่เริ่ม
...
- ⚠️ **Blocked by D1-B3** (ต้องมี RunRecord ก่อนถึง emit `CollectEvent` ได้จริง)

**Story D1-B3 · Decision Logging** `p0` `core` — ⬜ ยังไม่เริ่ม — **ทำก่อนสุด (blocking ทุก story ที่เหลือ)**
Log ทุก Y-Junction (decision 1–10): zone, choice, correct/systemic flag → เตรียม data ให้ D2-B2 (DAG)
- Task: `GamePlayScreen` ถือ RunRecord (single-writer) · emit events — **= PR3 ใน [ENGINEERING_PLAN.md](ENGINEERING_PLAN.md#dependency-graph--pr-strategy-gdd-v2)**
...
**Story D2-B2 · Stealth Assessment Score Engine** `p0` `core` — ⬜ ยังไม่เริ่ม
...
- ⚠️ **Blocked by D1-B3** (ต้องมี event log จริงให้อ่าน) — ค่าตัวเลข (systemic_point_c, boss_bonus_per_wave_c ฯลฯ) อยู่ใน `balance/v1/difficulty.json` แล้ว ไม่ต้องคิดใหม่

**Story D2-B3 · Auto-Generated DAG + Report Card Screen** `p0` `ui` — ⬜ ยังไม่เริ่ม (data พร้อมแล้ว)
...
- 📌 `balance/v1/dag.json` (13 edge, node/tooltip) **เสร็จแล้วจาก PR2** — เขียนแค่ `dag.py` (อ่าน json + evaluate เขียว/แดงจาก event log) + `report.py` (render)

**Story D2-B4 · Backend Sync Integration** `p1` `server` — ⬜ ยังไม่เริ่ม
