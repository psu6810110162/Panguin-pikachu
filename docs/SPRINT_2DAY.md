...
**Story D1-B2 · Evidence-to-Stats Item System** `p0` `game` — 🟡 **data model เสร็จ, ยังไม่ต่อสาย gameplay**
Item pickup (Albedo/Methane/Eco-Seed), inventory UI 3 ช่อง, Eco-Seed (Spacebar) ลด Heat + ซ่อมบล็อก
- Task: ~~inventory model~~ ✅ (`core/items.py::Inventory`, 3-slot, `tests/test_items.py`) · ⬜ extend pickup รองรับ `scientific_item` (ไม่มีที่ไหนเรียก `ItemType`/`Inventory(` นอก `core/items.py`+เทสต์) · ⬜ Eco-Seed action · ⬜ inventory UI
- ✅ Acceptance: เก็บ item ขึ้น inventory, ใช้ Eco-Seed แล้ว Heat ลดจริง
- ⚠️ **Blocked by D1-B3** (ต้องมี RunRecord ก่อนถึง emit `CollectEvent` ได้จริง) — D1-B3 เสร็จแล้ว ไม่ blocked แล้ว
...
**Story D2-B5 · History Screen Fix** `p2` `ui` — ⬜ ยังไม่เริ่ม (พบบั๊กจริงจาก user report 2026-07-14)
...
- Task: `db.get_last_player_name()` แทน hardcode, `ORDER BY distance_m DESC, played_at DESC`, เพิ่มเทสต์ (ดู plan `acknowledgement-hazy-octopus.md` Part A)

**Story D2-B6 · Classroom Session Join + Offline Sync (design doc)** `p1` `docs` — ⬜ ยังไม่เริ่ม
...
- Task: ดู plan `acknowledgement-hazy-octopus.md` Part B สำหรับโครง doc (user journey, ownership ADR-013, invariant local-save-first)
...
- **Milestone 2: Boss + Scoring (Day 2)** — Stories D2-A1 → D2-B6

## สถานะล่าสุด (2026-07-15, สรุปสั้นสำหรับ Dev B)

- **เพิ่งเสร็จ:** PR #70 (dedupe `PolicyChoiceEvent` ตาม zone, first-write-wins) — ตอบ Dev A ใน #46 แล้วเรื่อง duplicate-zone policy
- **PR ค้าง review:** #66, #67, #68, #69, #70
- **งานถัดไปตามลำดับความสำคัญที่ user ยืนยัน:** History fix (D2-B5) → Join/Sync design doc (D2-B6) → ต่อสาย client จริงเมื่อมี doc approved
- อ้างอิงแผนละเอียด: `~/.claude/plans/acknowledgement-hazy-octopus.md`
